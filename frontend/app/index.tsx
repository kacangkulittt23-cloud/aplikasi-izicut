...
export default function Dashboard() {
  ...
  const noData = yearsQ.isSuccess && (yearsQ.data?.length ?? 0) === 0;
  ...
  const Header = (
    <View>
      ...
      {!browsing ? (
        noData ? (
          <View style={styles.center} testID="empty-no-data">
            <Icon name="Users" size={44} color={colors.brandPrimary} />
            <Text style={styles.emptyTitle}>Belum ada data personil</Text>
            <Text style={styles.emptyText}>
              Masuk sebagai admin lalu unggah file Excel untuk mengisi basis data.
            </Text>
            <Pressable
            ...
            >
              <Icon name="UploadSimple" size={18} color={colors.onBrandPrimary} weight="bold" />
              <Text style={styles.importBtnText}>Buka Panel Admin</Text>
            </Pressable>
          </View>
        ) : (
          <DashboardBody
            stats={statsQ.data}
            loading={statsQ.isLoading || (activeYear != null && !statsQ.data)}
            onOpen={openProfile}
...
const useStyles = makeStyles((colors) => ({
  ...
  emptyText: { color: colors.muted, fontFamily: "DMSans", fontSize: 14, textAlign: "center", paddingHorizontal: 32 },
  emptyTitle: { color: colors.onSurface, fontFamily: "Oswald", fontSize: 20, letterSpacing: 0.5 },
  importBtn: {
    flexDirection: "row", alignItems: "center", gap: 8, marginTop: 8,
    backgroundColor: colors.brandPrimary, borderRadius: 999, paddingHorizontal: 20, paddingVertical: 12,
  },
  importBtnText: { color: colors.onBrandPrimary, fontFamily: "DMSans", fontSize: 15, fontWeight: "700" },
  pressed: { opacity: 0.7 },
